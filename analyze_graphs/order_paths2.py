	...
	for embed in list(embeds.keys()):
		for dist in list(dists.keys()):
			for truth_label in ['true','false']:
				write_path=os.path.join(graph_path,fcg_class,"paths",fcg_type+'_'+truth_label+"_({})".format(embed.split("/")[-1]))
	...
	for embed in list(embeds.keys()):
		for dist in list(dists.keys()):
			for truth_label in ['true','false']:
				...
				write_path=os.path.join(graph_path,fcg_class,"paths",tfcg_type+"_"+truth_label+"_({})".format(embed.split("/")[-1]))
				write_path=os.path.join(write_path,"paths")
				write_path=write_path+"_"+graph_type
				#mode can be weight/dist
				rw_path=write_path+"_"+dist
				...
				write_path=os.path.join(graph_path,fcg_class,"paths",ffcg_type+"_"+truth_label+"_({})".format(embed.split("/")[-1]))
				write_path=os.path.join(write_path,"paths")
				write_path=write_path+"_"+graph_type
				#mode can be weight/dist
				rw_path=write_path+"_"+dist
				...
				tfcg_paths={eval(claimID):{(str(tuple(sorted(eval(edge)[:2]))) if edge!='target_claim' else edge):({'tfcg':{edge:edge_paths}} if edge!='target_claim' else edge_paths) for edge,edge_paths in claim_edges.items()} for claimID,claim_edges in tfcg_paths.items()}
				ffcg_paths={eval(claimID):{(str(tuple(sorted(eval(edge)[:2]))) if edge!='target_claim' else edge):({'ffcg':{edge:edge_paths}} if edge!='target_claim' else edge_paths) for edge,edge_paths in claim_edges.items()} for claimID,claim_edges in ffcg_paths.items()}
				#Diff Calculate
				scores={}
				try:
					del ffcg_paths[10688]
				except:
					pass
				...
				scores_path=os.path.join(graph_path,fcg_class,"paths","scores_"+truth_label+"_"+dist+"_({})".format(embed))
				with codecs.open(scores_path+".json","w","utf-8") as f:
					f.write(json.dumps(scores,indent=6,ensure_ascii=False))
				np.save(scores_path+"_claimIDs2.npy",list(scores.keys()))
				for agg in list(aggs.keys()):
